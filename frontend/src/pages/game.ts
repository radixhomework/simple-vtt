...
import { parseStaticWalls, portalWalls, pathCrossesWall } from '../canvas/los'
...
export function renderGame(
    ...
    <div class="game">
      <div class="game-header">
        <div class="game-header-left">
          <button class="header-btn" id="back-btn">← Lobby</button>
  ...
  // Load map image
  if (table.map_image_path) {
    preloadMapImage(table.map_image_path, img => {
      state.mapImage = img
  ...
  function render() {
    ...
    rafId = requestAnimationFrame(() => {
      ...
      if (state.fogEnabled) {
        drawFog(fogCtx, state.tokens, state.fog, state.walls, state.camera, state.table.grid_size, isAdmin)
      }

      // UI canvas: measuring tool
  ...
  if (isAdmin) {
    const settingsPanel = root.querySelector('#settings-panel') as HTMLElement
    const settingsBody = root.querySelector('#settings-body') as HTMLElement
  ...
  uiCanvas.addEventListener('mousedown', (e) => {
    ...
    if (e.button === 0) {
      ...
      if (hit) {
        const canMove = isAdmin || hit.owner === user.username
        if (canMove) {
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'Escape') { state.selectedId = null; state.measure.active = false; refreshSidebar(); renderTokenEditor(); render() }
